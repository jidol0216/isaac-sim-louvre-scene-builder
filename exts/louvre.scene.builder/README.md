...
### 방법 2: Isaac Sim에서 직접 Extension 경로 추가
```bash
...
### 방법 3: 영구적으로 Extension 경로 추가
Isaac Sim 실행 후:
1. `Window` → `Extensions`
2. 상단의 ⚙️ (설정) 클릭
3. `Extension Search Paths`에 `/path/to/louvre_extension_minimal/exts` 추가
...
프로젝트 루트 기준:
- Louvre: `scenes/lubre.glb`
- Robot: `assets/robots/RidgebackFranka/ridgeback_franka.usd`
- Mecanum: `assets/wheel/basic_four_mecanum_robot_after_add_friction_for_rollers_2.usd`

**상대 경로를 사용하므로 어느 위치에 복사해도 동작합니다!**  
